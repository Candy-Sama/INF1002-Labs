...
def scale(list1, x):
    return list(map(lambda num: int(num * x) if (num * x).is_integer() else num * x, list1))

def sort(list1):
    return sorted(list1, key=lambda x: int(x) % 10)

def goodSales(list1):
    return list(filter(lambda x: x > sum(list1) / len(list1), list1))
...
def SalesAnalytics():
    ...
    try:
        ...
        sales_numbers = [int(num) if isinstance(num, int) or num.is_integer() else num for num in raw_numbers]
        scale_factor = float(sys.argv[2])
    except ValueError:
        print("Invalid input!")
        return

    scaled_numbers = scale(sales_numbers.copy(), scale_factor)
    sorted_numbers = sort(sales_numbers.copy())
    good_sales_numbers = goodSales(sales_numbers.copy())

    print(f"The scaled numbers are:  {scaled_numbers} The sorted sales numbers are:  {sorted_numbers} The good sales numbers are:  {good_sales_numbers}")
